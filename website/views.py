...
import json, os, sys, re, socket, datetime, time, subprocess
...
def ipDevices_status():
    ...
    data_ip2 = Ipdevices.query.all()
    status = []
    
    for data_ip5 in data_ip2:
         ip44 = data_ip5.ipaddress
         command = ['ping', '-n', '1', ip44]
    ...
    data_ip4 = Ipdevices.query.all()
    return render_template('devices.html', user=current_user, data_ip3= data_ip4 ,status=status)
...
#pinging added devices   
def ping_ip(ip):
	# to ping a particular IP
	try:
		socket.setdefaulttimeout(2)
		
		# if data interruption occurs for 1
		...
		host = ip
		port = 80 #53 for pinging server endpoints
...
def server_connectivity():
    ...
    render_template('servers.html',  user=current_user)
    #server_connectivity()
